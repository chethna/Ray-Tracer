...
public:
	...
	ALight(Vector3d, Vector3d, Vector3d, float, float, Color, float);
...
//		float y = p00.y+(n1*sy*randY);
	...
	ALight::ALight(Vector3d posValue, Vector3d n0Value, Vector3d n1Value, float sxValue,float syValue, Color colorValue, float intensityValue){
		position = posValue;
		n0 = n0Value;
		n1 = n1Value;
		sx = sxValue;
