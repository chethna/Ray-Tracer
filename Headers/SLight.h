...
public:
	...
	SLight(Vector3d, Vector3d, float, Color,float);
	...
	virtual double getCosTheta(Vector3d nh, Vector3d nlh){
		...
		double angleFormed = direction*(-1.0*nlh);
	...
	SLight::SLight(Vector3d posValue, Vector3d dirValue, float angleValue, Color colorValue, float intensityValue){
		position = posValue;
		direction = dirValue;
		angle = angleValue;
		color = colorValue;
