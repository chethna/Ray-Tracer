...
static bool irr =false;
static bool shaderBool = false;
static bool envMapBool = true;
...
double ambientLight = .9;

//defaults
static Vector3d ORIGIN(0,0,0);
static Vector3d Xaxis(1,0,0);
static Vector3d Yaxis(0,1,0);
static Vector3d Zaxis(0,0,1);
...
///////////////--------------------//////////////
				...
				Vector3d t1 = ORIGIN;
				RefReturn refreturn;
				float refIndex = scene_objects.at(index)->getRefractiveIndex();

				t1 = getTransmittedRay(intersecting_ray_dir,index_of_winning_object,refIndex);
				if (t1 == ORIGIN) {
					//total reflection
				}
				else{
					...
					if (newT<0) {
						...
						Vector3d newPhT = newPoint.getRayOrigin()+ newPoint.getRayDirection()*(newT);
						...
						//tottal internal reflection
							...
							if (index_of_winning_object_t>=0) {
								//cout << "inside reflection";
								//determine the postion and direction of the point of intersecton
								Vector3d pht = ray_t.getRayOrigin()+ ray_t.getRayDirection()*(intersections_t.at(index_of_winning_object_t)-0.0001);
								...
								totalColor_t = totalColor_t.colorScale(0.5);
							}
							
							
							else {
								if(envMapBool){
									totalColor_t = envMap(ray_t.getRayDirection(),ENV);
									//totalColor_r = totalColor_r.colorScale(0.5);
								}
							}
							
							final_color = final_color.colorScale(.2).colorAdd(totalColor_t.colorScale(.7));
						}
					}
				}
						///////////////-----------------//////////////////			
